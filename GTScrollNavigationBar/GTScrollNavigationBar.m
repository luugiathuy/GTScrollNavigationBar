...
//Uncomment the above & run to se what happens once you use a nav bar instead (thats your original code)

// through trial & error, i got shit to work but i have no clue why it works really
...
- (void)handlePan:(UIPanGestureRecognizer*)gesture
{
    ...
    if (isScrollingAndGestureEnded) {
        ...
        [self setFrame:frame alpha:alpha animated:NO];
        // had to the update the position of the scroll view frame every time the frame of the nav bar changed
        [self.scrollView setFrame:CGRectMake(frame.origin.x, frame.origin.y + 64, 320, 568 - 20)];
    }
